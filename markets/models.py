...
class Market(models.Model):
    ...
    embedding = models.JSONField(default=list, blank=True)  # 임베딩 저장
...
class SearchHistory(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    userId = models.ForeignKey(User, on_delete=models.CASCADE, related_name="searchhistory")
    marketId = models.ForeignKey(Market, on_delete=models.CASCADE, related_name="searchhistory")  # 검색된 상권
    createdAt = models.DateTimeField(auto_now_add=True)  # 검색 기록 생성 시간

    def __str__(self):
        return f"{self.userId.username} - {self.marketId} ({self.createdAt})"
